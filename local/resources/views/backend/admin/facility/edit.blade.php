...
@section('container')
    ...
    @endif
    {!! Form::model($facility,array('route' => ['facility.update',$facility->id],'method'=>'PATCH')) !!}
    <div class="col-md-12">
        <div class="row">
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                <div class="wrap-create-edit">
                    ...
                    @php
                        $localesPost=$translation->facilities()->get();
                            ...
                            @foreach($locales as $key=>$item)
                                @foreach($localesPost as $key2=>$item2)
                                    ...
                                    @if($item->id==$item2->locale_id)
                                        <option data-href="{{ route('facility.edit',$item2->id) }} "
                                                data-post-id="{{$item2->id}}" value="{{$item->id}}"
                                                @if($facility->locale_id==$item->id) selected @endif>{{$item->name}}</option>
                                    @endif
                                @endforeach
                            @endforeach
                            @foreach($locales as $key=>$item)
                    ...
                    <div class="group-more-lang">
                        @foreach($insertLangArray as $key=>$item)
                            <a href="{{ route('facility.createLocale',['translation_id'=>$facility->translation_id,'locale_id'=>$item->id]) }}">Thêm
        ...
        <div class="col-md-12">
            ...
            <div class="col-md-12" style="text-align:  center;">
                <button id="btnDanhMuc" type="submit" class="btn btn-primary">Cập Nhật Cơ Sở Vật Chất</button>
            </div>
        </div>
    </div>
    {!! Form::close() !!}
