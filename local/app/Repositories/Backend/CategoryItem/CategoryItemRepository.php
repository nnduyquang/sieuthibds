...
//        $result->categoryitems(CATEGORY_POST)->attach($attachData);
    ...
    {
        $data = [];
        $categoryItems = $this->find($id)->translations()->first()->facilities()->get();
        $translation = $this->find($id)->translations()->first();
        $arrayIDCategoryItems=array();
        foreach ($categoryItems as $key => $item) {
            array_push($arrayIDCategoryItems,$item->id);
            $this->delete($item->id);
        }
        $post=new Post();
        $posts=$post->getAllPost();
        foreach ($posts as $key=>$item){
            $item->facilities()->detach($arrayIDCategoryItems);
        }
